...
H = np.array([[1, 0, 0.6], [ 0, 1, 1.2], [0, 0, 1]])
...
F_output = fft2(output)
N, M = output.shape

subplot(222)
imshow(np.log(fftshift(np.abs(F_output)**2)),cmap = 'gray')
title('Original Frequency')

for coord, val in np.ndenumerate(F_output):
    F_output[coord[0], coord[1]] *= np.exp(-1j * 2 * np.pi * (coord[0] * 0.6/N + coord[1] * 1.2/M))
    
subplot(223)
imshow(np.log(fftshift(np.abs(F_output)**2)),cmap = 'gray')
title('Phase-Shifted Frequency')

inv_F_output = ifft2(F_output)

subplot(224)
imshow(np.abs(inv_F_output)**2, cmap = 'gray')
title('Translated Image')
show()'''
