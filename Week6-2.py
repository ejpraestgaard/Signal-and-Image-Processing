...
reshaped_peaks = peaks2.reshape(900,2)
taus = np.array([[(100 + i) // 100] for i in range(900)])

advanced_peaks = np.append(reshaped_peaks, taus, axis = 1)

coords = np.unique(reshaped_peaks, axis = 0, return_index = True)

uniques = [advanced_peaks[i] for i in range(len(coords[1]))]
...
for i in uniques:
    add_circle(hand2, i[0], i[1], 3 * i[2])


# In[1262]:


handcircles = plt.figure()

imshow(hand2)
axis('off')

handcircles.savefig('handcircles.jpg')
